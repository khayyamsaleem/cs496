(module checker (lib "eopl.ss" "eopl")
        ...
        (define type-of
          (lambda (exp tenv)
            (cases expression exp
                   ...
                   (setref-exp (le re)
                      (let ((e-type (type-of le tenv)))
                        (cases type e-type
                               (ref-type (arg-type) (unit-type))
                               (else (report-setref le)))))

                   (for-exp (id lb up body) (eopl:error "not implemented!"))

                   (type-exp (id cs ds) (eopl:error "not implemented!"))
        ...
        (define report-setref
          (lambda (arg)
            (eopl:error 'type-of-expression
                   "Address of setref is not a ref-type: ~% ~s" arg)))
        (define report-deref
          (lambda (arg)
            (eopl:error 'type-of-expression
                        "Address of deref is not a ref-type: ~% ~s" arg)))
